import '../entities/official_entities_service.dart';
...
class IntentDetectionService {
  ...
  static IntentAnalysisResult analyzeIntent(String message, String sender) {
    ...
    bool hasSuspiciousLinks = links.isNotEmpty &&
        links.any((link) => OfficialEntitiesService.isLinkSuspicious(link, entities));
    ...
    // RULE 0: Financial action + Suspicious link = BLOCK (CRITICAL)
    // This catches: "Paga $X en http://fake-site.com" regardless of other signals
    if (intents.contains(MessageIntent.FINANCIAL_ACTION) && hasSuspiciousLinks) {
      action = RecommendedAction.BLOCK_AND_SHOW_OFFICIAL;
      guidance = entities.isNotEmpty
          ? _buildBlockGuidance(entities, intents)
          : '🚫 Este mensaje solicita dinero con un enlace no verificado. NUNCA pagues por enlaces de SMS.';
      blockLinks = true;
      riskBoost = 50;
    }

    // RULE 1: Credential request + Suspicious link = BLOCK
    else if (intents.contains(MessageIntent.CREDENTIAL_REQUEST) && hasSuspiciousLinks) {
      action = RecommendedAction.BLOCK_AND_SHOW_OFFICIAL;
      guidance = entities.isNotEmpty
          ? _buildBlockGuidance(entities, intents)
          : '🚫 Este mensaje solicita credenciales con un enlace sospechoso. Posible phishing.';
      blockLinks = true;
      riskBoost = 50;
    }

    // RULE 2: High-risk intent + Entity + Official link = WARN (not BLOCK)
    // This allows: "Paga factura Bancolombia en www.bancolombia.com"
    else if (_isHighRiskIntent(intents) && entities.isNotEmpty && links.isNotEmpty && !hasSuspiciousLinks) {
      action = RecommendedAction.WARN_AND_SUGGEST;
      guidance = '⚠️ Mensaje de ${entities.map((e) => e.name).join(', ')} con enlace oficial. Verifica que sea esperado.';
      blockLinks = false;
      riskBoost = 20;
    }

    // RULE 3: High-risk intent + Entity mention + Call-to-action (no link) = WARN
    else if (_isHighRiskIntent(intents) && entities.isNotEmpty && hasCallToAction) {
      action = RecommendedAction.WARN_AND_SUGGEST;
      guidance = 'Este mensaje solicita una acción. Verifica con los canales oficiales de ${entities.first.name}.';
      blockLinks = false;
      riskBoost = 30;
    }

    // RULE 4: Urgency + Financial + Link = BLOCK (even without entity)
